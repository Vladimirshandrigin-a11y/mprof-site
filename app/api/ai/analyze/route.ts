...
function buildPrompt(d: SanitizedData): { system: string; user: string } {
  const system = [
    ...
    "",
    "Запрещено:",
    "— очевидные советы без пользы («себестоимость высокая», «логистика высокая»);",
    "— «проверьте данные» без указания, что именно проверить;",
    "— «увеличьте цену» без причины из цифр;",
    "— «снизьте расходы» без указания, какие именно расходы;",
