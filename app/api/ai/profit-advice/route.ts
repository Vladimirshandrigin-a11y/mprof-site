...
// Значение приходит готовым с фронта (его ВРУЧНУЮ выбрал пользователь).
// Backend НИЧЕГО не вычисляет — только валидирует по фиксированному набору;
...
function buildPrompt(d: SanitizedData): { system: string; user: string } {
  ...
  const system = [
    ...
    "",
    "СХЕМА ДОСТАВКИ (FBO / FBS / realFBS) — пользователь ВРУЧНУЮ выбрал её, значение в поле «схемаДоставки». Учитывай его и НЕ выдумывай схему:",
    ...
    "- fbs: товар лежит на складе продавца, а доставку по стране выполняет маркетплейс. Советы — про упаковку, габариты, сборку и обработку заказов и доставку до склада/сортировки маркетплейса.",
    "- realfbs (realFBS): товар лежит у продавца И доставку покупателю выполняет сам продавец (свои курьеры/службы, без логистики маркетплейса). Советы — про собственную доставку, стоимость курьеров/служб, сроки и зоны доставки, упаковку и обработку заказов.",
    "- mixed: используются несколько схем сразу. Прямо напиши, что схема смешанная, и что советы по логистике надо проверять отдельно по каждой схеме.",
    "- unknown: схема не выбрана. НЕ предполагай схему. Если речь заходит о доставке — напиши «схема доставки не указана» и дай нейтральные советы без привязки к конкретной схеме.",
    "- Схема доставки — это пользовательский контекст. НЕ меняй расчётные показатели (выручку, прибыль, маржу и т.п.) — они уже посчитаны.",
    "",
