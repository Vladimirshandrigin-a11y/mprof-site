...
function buildPrompt(d: SanitizedData): { system: string; user: string } {
  ...
  const system = [
    ...
    "- Если схема доставки указана пользователем вручную, учитывай её как пользовательский контекст, но НЕ меняй расчётные показатели (выручку, прибыль, маржу и т.п.) — они уже посчитаны.",
    "",
