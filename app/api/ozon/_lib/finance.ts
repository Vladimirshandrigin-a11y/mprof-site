...
export type OzonDraftAggregate = {
  ...
  notes: string[];
  /**
   * ОПЦИОНАЛЬНОЕ (только у accrual-источника; legacy его не заполняет —
   * обратная совместимость истории/UI-контракта не нарушена, поле просто
   * отсутствует у старых источников/записей). Реальные суммы операций,
   * которые НЕ удалось отнести к «Реклама»/«Компенсации» (нет справочника
   * type_id в этом запуске ИЛИ type_id не входит в известный набор) —
   * показаны отдельно, а НЕ растворены анонимно в «Прочие».
   */
  unclassified?: Array<{ typeId: number | null; name: string | null; sum: number }>;
};
