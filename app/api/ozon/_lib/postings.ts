...
export type OzonPostingItem = {
   ...
   * "" если Ozon не вернул статус. Боевой расчёт себестоимости
   * (aggregateProfitCostDraft) включает в COGS ТОЛЬКО отправления со статусом
   * "delivered". Диагностика (aggregateCostByStatus) раскладывает себестоимость
   * по ВСЕМ статусам, чтобы было видно, что именно исключено.
   */
  status: string;
};
 ...
 * В себестоимость (COGS) включаются ТОЛЬКО товары из ДОСТАВЛЕННЫХ отправлений
 * (status === "delivered"); отменённые, в пути, ожидающие и отправления без статуса
 * в COGS НЕ входят. Возвращает покрытие (matched/unmatched по доставленным),
 * Σ себестоимости сопоставленных с ценой, список «без учёта стоимости» и топ по
 * стоимости. Никакого fuzzy и никакой выдуманной себестоимости. ПРИБЫЛЬ НЕ СЧИТАЕМ.
 */
export function aggregateProfitCostDraft(
  ...
  // === COGS — ТОЛЬКО доставленные отправления (status === "delivered") ===
  // Себестоимость списываем лишь по товарам из ДОСТАВЛЕННЫХ отправлений. Отменённые
  // (cancelled), в доставке (delivering/in_transit), ожидающие (awaiting_*) и
  // отправления с пустым/неизвестным статусом в COGS НЕ входят — иначе себестоимость
  // завышается, а чистая прибыль занижается. Полную разбивку по статусам (включая
  // невошедшие) показывает диагностика aggregateCostByStatus — её логику НЕ трогаем.
  const deliveredItems = items.filter((it) => it.status === "delivered");
  ...
  // Агрегируем ДОСТАВЛЕННЫЕ позиции в уникальные товары (offer_id → sku → без идентификатора).
  const agg = new Map<string, AggItem>();
  ...
  if (coverage.uniqueOzonItems === 0) {
    notes.push(
      items.length > 0
        ? "За выбранный месяц есть отправления Ozon, но ни одно не доставлено (delivered) — себестоимость по доставленным равна 0."
        : "За выбранный месяц отправления Ozon не найдены — себестоимость считать не из чего."
    );
  } else {
    notes.push(
      "В себестоимость включены ТОЛЬКО доставленные отправления (delivered). Отменённые и недоставленные отправления не списываются."
    );
    notes.push(
...
//
// Зачем: видеть, из каких статусов складывается себестоимость отправлений и сколько
// «весят» НЕ доставленные статусы. Боевой API-расчёт (aggregateProfitCostDraft) теперь
// включает в COGS ТОЛЬКО доставленные отправления (delivered); эта диагностика
// раскладывает себестоимость по ВСЕМ статусам (GROSS) — чтобы было видно, что именно
// исключено (cancelled / в пути / ожидание). Она НИЧЕГО не фильтрует и боевой расчёт
// НЕ меняет — только раскладывает суммы по статусам.
//
// Матч идентичен боевому: offer_id↔products.sku, затем точное Ozon-sku↔products.sku,
// учитываем стоимость только при cost_price > 0. Здесь totalMatchedCost — это GROSS
// по всем статусам; боевой COGS = deliveredMatchedCost (только delivered).
// ---------------------------------------------------------------------------
...
export type CostByStatus = {
  /** Σ сопоставленной себестоимости по ВСЕМ статусам (GROSS). Боевой COGS = deliveredMatchedCost. */
...
export function aggregateCostByStatus(
  ...
  if (items.length === 0) {
    ...
    notes.push(
      "Разбивка себестоимости по статусам — диагностика (GROSS, все статусы). Боевой API-расчёт включает в себестоимость ТОЛЬКО «Доставлен» (delivered); «Отменён» и прочие недоставленные статусы в расчёт не входят и не списываются."
    );
    notes.push(
