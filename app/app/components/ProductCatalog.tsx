...
export function ProductCatalog({ user, showToast }: Props) {
  ...
  // Порядок отображения каталога: сначала по НАЗВАНИЮ, при равных названиях —
  // по АРТИКУЛУ/SKU. Чисто визуальная сортировка (только отображение): данные в
  // Supabase не меняются. Копия массива ([...products]) → sort НЕ мутирует
  // исходный state. Зависит только от products → пересортировывается после
  // загрузки/добавления/импорта/редактирования/удаления, но НЕ при правке
  // себестоимости в строке. На сохранение, формулы и другие блоки не влияет.
  //
  // Нормализация перед сравнением: trim + нижний регистр. localeCompare с
  // русской локалью и { sensitivity: "base", numeric: true } → корректный
  // алфавит (е/ё, регистр игнорируются) и ЕСТЕСТВЕННЫЙ порядок чисел
  // («5м» раньше «10м», «SKU 2» раньше «SKU 10»). Товары без названия уходят
  // ВНИЗ списка, но их SKU всё равно участвует в сортировке между собой.
  const sortedProducts = useMemo(() => {
    const norm = (s: string | null | undefined) => (s ?? "").trim().toLowerCase();
    const cmp = (x: string, y: string) =>
      x.localeCompare(y, "ru", { sensitivity: "base", numeric: true });
    return [...products].sort((a, b) => {
      const an = norm(a.name);
      const bn = norm(b.name);
      // Безымянные товары — в конец (SKU учитываем отдельной веткой ниже).
      if (an && !bn) return -1;
      if (!an && bn) return 1;
      const byName = cmp(an, bn);
      if (byName !== 0) return byName;
      // Названия совпали (или оба пустые) → сортируем по SKU/артикулу.
      return cmp(norm(a.sku), norm(b.sku));
    });
  }, [products]);
