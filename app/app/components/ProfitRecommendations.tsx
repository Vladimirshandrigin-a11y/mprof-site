...
export function ProfitRecommendations(props: ProfitRecommendationsProps) {
        ...
        <div className="pr-verdict ok">
          ...
          <div className="pr-verdict-bd">
            ...
            <p className="pr-verdict-txt">
              Отчёт загружен: выручка{accrualCategories ? " после возвратов" : ""}{" "}
              {fmtRub(revenue)}, к перечислению от Ozon{" "}
  ...
  if (netProfit < 0) {
    ...
    verdict = `Чистый убыток ${fmtRub(Math.abs(netProfit))} при выручке${acc ? " после возвратов" : ""} ${fmtRub(
      revenue
    ...
    verdict = `Чистая прибыль ${fmtRub(netProfit)}. Маржа не определяется, потому что выручка после возвратов не положительна, — оценивайте результат по абсолютной прибыли и структуре расходов ниже.`;
  } else if (margin < 5) {
    tone = "warn";
    ...
    checks.push({
      ...
      text: acc
        ? `Списания Ozon по отчёту начислений (комиссия, доставка, реклама, сборы) забирают ${pct(
            updShare
          )} выручки после возвратов с учётом баллов за скидки — сверьте тарифы и категории начислений.`
